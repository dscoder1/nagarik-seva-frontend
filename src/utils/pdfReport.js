...
export function generatePDF({ role, reportData, user, period, from, to, lang = 'en' }) {
  ...
  doc.setFontSize(20);
  doc.setTextColor(...COLORS.white);
  doc.text(isHindi ? 'नागरिकसेवा' : 'NagarikSeva', M + 26, 17);
  ...
  doc.text('NagarikSeva — Civic Complaint Management System', M, footerY + 5);
  ...
  const fileName = `NagarikSeva_${roleStr}_Report_${new Date().toISOString().slice(0,10)}.pdf`;
  doc.save(fileName);
}
